...
def create_vocab(trainqa_path, answerset_path, vocab_path, video_id_path):
    ...
    video_ids = pd.read_csv(video_id_path, header=None)[0]
    train_qa = train_qa[train_qa['answer'].isin(answerset)]

    train_qa = train_qa[train_qa['video_id'].isin(video_ids)]
...
def create_qa_encode(vttqa_path, vocab_path, answerset_path,
    ...
    val_qa = val_qa[val_qa['video_id'].isin(video_ids)]
    test_qa = pd.read_json(os.path.join(vttqa_path, 'test_qa.json'))
    test_qa = test_qa[test_qa['video_id'].isin(video_ids)]
